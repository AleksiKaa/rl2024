...
def create_env(config_file):
    ...
    register(
        ...
        + ":SandingEnv",  # Updated to use __name__ for current module
    ...
    env = gym.make(
        cfg["env_name"], render_mode=cfg["env_config"].get("render_mode", "rgb_array")
    )
    return env
...
def plot_algorithm_training(path, seeds, env_name, algo_name):
    ...
    # Calculate mean and std across different runs
    steps, mean_average_return, std_average_return = get_statistical_plots_data(
        path, seeds, env_name, algo_name
    )
    ...
    plt.fill_between(
        steps[1:],
        mean_average_return[1:] - std_average_return[1:],
        mean_average_return[1:] + std_average_return[1:],
        alpha=0.2,
        label="Std Dev",
    )
...
def compare_algorithm_training(algo1, algo2, seeds):
    steps1, mean_average_return1, std_average_return1 = get_statistical_plots_data(
        algo1.logging_dir, seeds, algo1.env_name, algo1.algo_name
    )
    steps2, mean_average_return2, std_average_return2 = get_statistical_plots_data(
        algo2.logging_dir, seeds, algo2.env_name, algo2.algo_name
    )
    ...
    plt.plot(
        steps1[1:],
        mean_average_return1[1:],
        linewidth=1.2,
        label=f"{algo1.cfg.algo_name}",
    )

    # Plot std deviation area for algo1
    plt.fill_between(
        steps1[1:],
        mean_average_return1[1:] - std_average_return1[1:],
        mean_average_return1[1:] + std_average_return1[1:],
        alpha=0.2,
    )
    ...
    plt.plot(
        steps2[1:],
        mean_average_return2[1:],
        linewidth=1.2,
        label=f"{algo2.cfg.algo_name}",
    )

    # Plot std deviation area for algo2
    plt.fill_between(
        steps2[1:],
        mean_average_return2[1:] - std_average_return2[1:],
        mean_average_return2[1:] + std_average_return2[1:],
        alpha=0.2,
    )
